...
export const fetchDishes = (slug: string) =>
  axios.get<DishModel[]>(`/api/orders/${slug}/dishes`).then(({ data }) => data);

export const markAsClosed = ({ slug }: OrderModel) =>
  axios
    .post<OrderModel>(`/api/orders/${slug}/mark-as-closed`)
    .then(({ data }) => data);

export const markAsOpen = ({ slug }: OrderModel) =>
  axios
    .post<OrderModel>(`/api/orders/${slug}/mark-as-open`)
    .then(({ data }) => data);

export const setPayer = ({ slug }: OrderModel, setPayerDto: SetPayerDto) =>
  axios
    .post<OrderModel>(`/api/orders/${slug}/set-payer`, {
      data: setPayerDto,
    })
    .then(({ data }) => data);
...
export const markAsDelivered = ({ slug }: OrderModel) =>
  axios
    .post<OrderModel>(`/api/orders/${slug}/mark-as-delivered`)
    .then(({ data }) => data);

export const markAsOrdered = ({ slug }: OrderModel) =>
  axios
    .post<OrderModel>(`/api/orders/${slug}/mark-as-ordered`)
    .then(({ data }) => data);
export const markAsPaid = ({ slug }: OrderModel, payerId: number) =>
  axios
    .post<OrderModel>(`/api/orders/${slug}/mark-as-paid`, {
      payerId,
    })
    .then(({ data }) => data);
...
export const addDish = ({ slug }: OrderModel, addDishDto: AddDishDto) =>
  axios
    .post<DishModel>(`/api/orders/${slug}/dishes`, addDishDto)
    .then(({ data }) => data);

export const deleteDish = ({ slug }: OrderModel, dishId: number) =>
  axios
    .delete<DishModel>(`/api/orders/${slug}/dishes/${dishId}`)
    .then(({ data }) => data);
...
export const updateDish = (
  ...
  axios
    .put<DishModel>(`/api/orders/${slug}/dishes/${dishId}`, addDishDto)
    .then(({ data }) => data);
