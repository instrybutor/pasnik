...
export function useOrderDish(order: OrderModel, dish: DishModel) {
  ...
  const onDuplicate = useCallback(async () => {
    ...
    toast({ type: 'success', title: t('v2-order.toast.duplicate_dish') });
  }, [t, dish, addDishAsync, toast]);

  const onDelete = useCallback(async () => {
    await deleteDishAsync();
    toast({ type: 'success', title: t('v2-order.toast.delete_dish') });
  ...
  const onUpdate = useCallback(
    async (data: AddDishDto) => {
      ...
      toast({ type: 'success', title: t('v2-order.toast.update_dish') });
    },
